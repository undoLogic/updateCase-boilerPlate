...
VISUAL_ABSOLUTE_PATH=$(grep '^ *"VISUAL_ABSOLUTE_PATH":' settings.json  | awk '{ print $2 }' | sed -e 's/,$//' -e 's/^"//' -e 's/"$//')
VISUAL_URL=$(grep '^ *"VISUAL_URL":' settings.json  | awk '{ print $2 }' | sed -e 's/,$//' -e 's/^"//' -e 's/"$//')

#
#
#
#Rsync the files from test location to LIVE
ssh $SSH_USER@$SSH_HOST "rsync -av --omit-dir-times --no-perms $TESTING_ABSOLUTE_PATH/$GITHUB_CURRENT_BRANCH/$SRC_FILES_RELATIVE_PATH/. $VISUAL_ABSOLUTE_PATH/." && echo ""
#open firefox new tab with link
"C:\Program Files\Firefox Developer Edition\firefox.exe" -new-tab $VISUAL_URL